...
// ── Floating panel (iframe overlay) ───────────────────────────────────────────
const PANEL = {
  visible: false,

  build() {
    if (document.getElementById('gct-panel-wrap')) return;

    const iconUrl = chrome.runtime.getURL('icons/CaptureText_icon.png');
    const popupUrl = chrome.runtime.getURL('popup/popup.html');

    const wrap = document.createElement('div');
    wrap.id = 'gct-panel-wrap';
    wrap.innerHTML = `
      <div id="gct-drag-bar">
        <img id="gct-drag-bar-icon" src="${iconUrl}" alt="">
        <span class="gct-drag-title">文字捕獲工具</span>
        <span class="gct-drag-grip">⠿⠿</span>
        <button id="gct-panel-close-btn" title="關閉面板">✕</button>
      </div>
      <iframe id="gct-panel-iframe" src="${popupUrl}"></iframe>
    `;
    document.documentElement.appendChild(wrap);

    // ── Close button ───────────────────────────────────────────────────────
    document.getElementById('gct-panel-close-btn').addEventListener('click', () => this.hide());

    // ── Close message from inside iframe (popup.js sends this) ─────────────
    window.addEventListener('message', e => {
      if (e.data?.type === 'GCT_CLOSE') this.hide();
    });

    // ── Draggable (iframe pointer-events trick) ────────────────────────────
    const dragBar = document.getElementById('gct-drag-bar');
    const iframe  = document.getElementById('gct-panel-iframe');
    let dragging = false, offX = 0, offY = 0;

    dragBar.addEventListener('mousedown', e => {
      if (e.target.id === 'gct-panel-close-btn') return;
      dragging = true;
      const rect = wrap.getBoundingClientRect();
      offX = e.clientX - rect.left;
      offY = e.clientY - rect.top;
      wrap.style.right = 'auto';
      iframe.style.pointerEvents = 'none'; // prevent iframe swallowing mousemove
      e.preventDefault();
    });
    document.addEventListener('mousemove', e => {
      if (!dragging) return;
      const x = Math.min(Math.max(0, e.clientX - offX), window.innerWidth  - wrap.offsetWidth);
      const y = Math.min(Math.max(0, e.clientY - offY), window.innerHeight - wrap.offsetHeight);
  ...
  show() {
    this.build();
    const wrap = document.getElementById('gct-panel-wrap');
    if (wrap) {
      wrap.classList.remove('gct-panel-hidden');
      this.visible = true;
    }
  },
  ...
  hide() {
    const wrap = document.getElementById('gct-panel-wrap');
    if (wrap) {
      wrap.classList.add('gct-panel-hidden');
      this.visible = false;
    }
  },
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  // ── Synchronous: panel toggle ────────────────────────────────────────────
  if (msg.type === 'TOGGLE_PANEL') {
    PANEL.toggle();
    sendResponse({ ok: true, visible: PANEL.visible });
    return false;
  }
